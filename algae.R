...
#							   'NO3','NH4','oPO4','Chal','a1','a2','a3','a4','a5',
...
plot(algae$NH4, xlab = "")
clicked.line <- identify(algae$NH4)
algae[clicked.line,]
...
cv.rf <- function(form, train, test, ...) {
	...
	p <- predict(m, test)
	mse <- mean((p - resp(form, test))^2)
	c(nmse = mse/mean((mean(resp(form, train)) - resp(form, test))^6))
}
...
#clean.test.algae$size <- revalue(clean.test.algae$size, 
#								c("large"="large_","small"="small_"))
#clean.test.algae$speed <- revalue(clean.test.algae$speed, 
#								c("low"="low__","high"="high__"))
